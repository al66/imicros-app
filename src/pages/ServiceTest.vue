    ...
    <q-tab-panels v-model="tab">
      ...
      >
        ...
        <div :class="{ 'truncate-chip-labels': true }">
          <q-chip dense>
            {{ serviceSelected ? ( serviceSelected.method || 'POST' ) + ' ' + baseUrl + serviceSelected.value : baseUrl }}
          </q-chip>
          <q-chip
            v-if="serviceSelected.git"
            icon="bi-github"
            :label="serviceSelected.git"
            clickable
            dense
            @click="git(serviceSelected.git)"
          />
        </div>
        <q-splitter
  ...
  methods: {
    call () {
      ...
      if (index >= 0) {
        ...
        // console.log(params)
        instance.request({ method: this.service.method || 'post', url: '/api/' + this.service.value, params }).then((result) => {
          // console.log(result)
          if (result) {
            if ('data' in result) {
              try {
