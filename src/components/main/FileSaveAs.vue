    ...
    <q-dialog v-model="dialog">
      <q-card style="width: 500px;">
        <q-card-section align="center" class="bg-black text-white q-pa-xs">
          <div class="text-h6">{{ $t('Files.save.as.dialog.title') }}</div>
        </q-card-section>
        ...
        <q-card-section>
          <q-input v-model="objectName" :prefix="path" :suffix="suffix" :label="$t('Files.save.as.dialog.input.label')" clearable @keyup.enter="saveAs()"></q-input>
        </q-card-section>
        <q-card-actions align="right">
          <q-btn dense flat :label="$t('Action.save.as')" icon="ion-save" text-color="primary" :disable="!objectName || objectName.length === 0" @click="saveAs()"/>
  ...
  methods: {
    ...
    select (object) {
      ...
      if (object.name) {
        ...
        if (this.suffix) {
          let pos = this.objectName.lastIndexOf(this.suffix)
    ...
    saveAs () {
      ...
      let objectName = this.path + this.objectName + this.suffix
      this.objectName = ''
